...
configtitle = "FullScale TWR=4 Iterative Solution 5_28_23"
output=True
...
while rocket.h < targetH+10 or rocket.h > targetH + 500:
    print(f"got to {rocket.h} at impulse {impulse} and mi = {structure.mis}")
    deltaimpulse = (impulse*targetH/rocket.h-impulse)*.75
    impulse = impulse+deltaimpulse
    miinittemp = 0
    mi = structure.mis
    while abs(miinittemp - mi) > 2:
        miinittemp = mi
        params['thrust'] = structure.totalmasses * thrustToWeight * 9.81  # recompute with a resonable thrust
        ...
        structure.Equilibrium(params,12)
        params['dp'] = structure.dparray
        mi = structure.mis
        print(f"MI is {mi}, Thrust is {params['thrust']} and dp is {params['dp']}")
    rocket.Equilibiurm(.25)
newargs = {
