...
def Plotter(hlist, vlist, thrustlist, isplist, machlist, time=None, title=None, dt=None):
    ...
    fig.suptitle(title)
    axs[0, 0].plot(np.arange(0, (np.where(hlist == 0)[0][1]) * dt - dt / 2, dt), hlist[0:np.where(hlist == 0)[0][1]],
                   'g')  # row=0, column=0
    axs[1, 0].plot(np.arange(0, (np.where(hlist == 0)[0][1] * dt - dt / 2), dt), vlist[0:np.where(hlist == 0)[0][1]],
                   'b')  # row=1, column=0
    axs[0, 1].plot(np.arange(0, np.where(hlist == 0)[0][1] * dt - dt / 2, dt), alist[0:np.where(hlist == 0)[0][1]],
                   'm')  # row=0, column=0
    axs[1, 1].plot(np.arange(0, np.where(hlist == 0)[0][1] * dt - dt / 2, dt), machlist[0:np.where(hlist == 0)[0][1]],
                   'k')  # row=1, column=0
...
def rocketEquationCEA_MassAprox(params, impulse, thrustToWeight, H, dp, dt=None, Af=None, ispcorrection=None):
    ...
    while abs(miinittemp - mi) > 2:
        miinittemp = mi
        mi, L, totalmass, wstruct, newheight, heightox, heightfuel, vol_ox, vol_fuel, P_tank = SA.mass_approx(
            params['pc'], params['dp'], 12, params['rho_fuel'], params['rho_ox'],
